...
You've learned to copy files with `cp` and create directories with `mkdir`. Now it's time to master one of the most versatile file management commands in Linux: `mv`. Unlike copying, which creates duplicates, `mv` **moves** files and directories—which also makes it perfect for **renaming** them.
...
The simplest use of `mv` is renaming—moving a file to a new name in the same location.
...
- No copying occurred—the file was simply renamed
...
💡 **Renaming is just moving within the same directory**—that's why the same command handles both operations!
</Callout>
...
⚠️ **mv requires write permission to the source directory** because it's removing the file from there. This is different from `cp`, which only needs read permission on the source.
</Callout>
...
<Callout type="info">
💡 Remember: wildcards are expanded by the shell **before** `mv` runs. The `mv` command sees a list of files, not the wildcard pattern.
</Callout>
...
<div className="grid grid-cols-1 md:grid-cols-2 gap-6 my-8">
  <div className="bg-white dark:bg-gray-800 p-6 rounded-lg border-2 border-blue-200 dark:border-blue-900">
    ...
    <ul className="space-y-2 text-sm">
      <li>✓ Creates duplicate</li>
      <li>✓ Original remains</li>
      <li>✓ Requires -r for directories</li>
      <li>✓ Only needs read permission on source</li>
      <li>✓ Slower (reads and writes data)</li>
      <li>✓ Uses disk space (duplicate data)</li>
      <li>✓ Works across filesystems freely</li>
    </ul>
  </div>
  <div className="bg-white dark:bg-gray-800 p-6 rounded-lg border-2 border-green-200 dark:border-green-900">
    ...
    <ul className="space-y-2 text-sm">
      <li>✓ Moves original</li>
      <li>✓ Original disappears</li>
      <li>✓ No -r needed for directories</li>
      <li>✓ Needs write permission on source directory</li>
      <li>✓ Instant (just updates metadata, same filesystem)</li>
      <li>✓ No extra disk space used</li>
      <li>✓ Slower across filesystems (becomes copy+delete)</li>
    </ul>
  </div>
</div>
...
<Callout type="warning">
⚠️ **Never move critical system files** like `/etc/passwd`, `/etc/shadow`, or `/boot/vmlinuz` unless you absolutely know what you're doing. You can break your system!
</Callout>
...
<Callout type="success">
🎯 **Hands-on practice** is crucial for mastering `mv`. These 20 labs progress from basic to advanced real-world scenarios.
</Callout>
...
**Key concept**: `-v` shows you exactly what `mv` did—useful for audit trails.
</details>
...
<table className="min-w-full border-collapse border border-gray-300 dark:border-gray-700 my-6">
  <thead>
    <tr className="bg-gray-50 dark:bg-gray-800">
      <th className="border border-gray-300 dark:border-gray-700 px-4 py-2 text-left">DO ✅</th>
      <th className="border border-gray-300 dark:border-gray-700 px-4 py-2 text-left">DON'T ❌</th>
    </tr>
  </thead>
  <tbody>
    <tr className="hover:bg-gray-50 dark:hover:bg-gray-800">
...
Moving important files?
  └─ Use mv -i (interactive)

Moving in script?
  └─ Use mv -n (no clobber) or mv -u (update)

Need audit trail?
  └─ Use mv -v (verbose)

Moving directories?
  └─ Just mv dirname/ dest/ (no -r needed)

Cross-filesystem move?
  └─ Expect slower operation (copy + delete)
```
...
<Callout type="success">
🎉 **Congratulations!** You now understand how to move and rename files efficiently with `mv`. You've learned:
...
**Practice these labs until mv operations become second nature**—you'll use this command constantly as a Linux administrator! 🚀
</Callout>
