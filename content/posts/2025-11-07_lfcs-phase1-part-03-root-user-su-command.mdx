...
<Callout type="info">
💡 **Critical Fact**: The magic of root is **UID 0**, not the username. Any account with UID 0 has full root privileges, regardless of its name. The kernel only checks the UID!
</Callout>
...
⚠️ **With great power comes great responsibility!** As root, there's no "Are you sure?" prompt. `rm -rf /` will delete your entire system without hesitation.
</Callout>
  ...
  <div className="space-y-4">
    <div className="bg-blue-100 dark:bg-blue-900 p-4 rounded">
      ...
      <div className="text-sm pl-4">
        <div>• Regular User (UID 1000): Limited permissions</div>
        <div>• Root User (UID 0): Unlimited permissions</div>
      </div>
    </div>
    <div className="text-center text-2xl">↓ System Calls ↓</div>
    <div className="bg-purple-100 dark:bg-purple-900 p-4 rounded">
      <div className="font-semibold mb-2">Linux Kernel</div>
      <div className="text-sm pl-4">
        <div>• Checks UID of calling process</div>
        <div>• If UID = 0 → Allow operation</div>
        <div>• If UID ≠ 0 → Check permissions</div>
      </div>
    </div>
    <div className="text-center text-2xl">↓ Hardware Access ↓</div>
    <div className="bg-green-100 dark:bg-green-900 p-4 rounded">
      <div className="font-semibold mb-2">Hardware Resources</div>
      <div className="text-sm pl-4">
        <div>• CPU, Memory, Disk, Network</div>
        <div>• Only kernel can access directly</div>
      </div>
    </div>
  </div>
</div>
...
1. **What is the UID of this process?**
   ...
   - If UID ≠ 0: Check file permissions, capabilities, and other restrictions
...
✅ **For LFCS**: Understand that root's power comes from UID 0, which the kernel treats specially. This is fundamental to Linux security and privilege escalation.
</Callout>
  ...
  <table className="min-w-full border-collapse border border-gray-300 dark:border-gray-700">
    ...
    <tbody>
      ...
      <tr className="hover:bg-gray-50 dark:hover:bg-gray-800">
        ...
        <td className="border border-gray-300 dark:border-gray-700 px-4 py-2">❌ Rarely</td>
        <td className="border border-gray-300 dark:border-gray-700 px-4 py-2">✅ Almost always</td>
      </tr>
    </tbody>
  </table>
</div>
...
<Callout type="warning">
⚠️ **Always use `su -` with the dash!** Using `su` without the dash can cause problems because commands may not be in your PATH, and configuration files won't be loaded properly.
</Callout>
...
### ✅ Root User and su Best Practices
...
<Callout type="success">
🎉 **Congratulations!** You've completed Part 3 of the LFCS Certification series. You now understand the Linux root user, the difference between su and su -, and how the kernel treats UID 0.
