...
The most basic usage—simply run `hostname` with no arguments:
...
<Callout type="info">
💡 **Note**: If you don't have a domain configured, `hostname -f` will just return your hostname (same as `hostname` without options). This is common on home networks and virtual machines without DNS configuration.
</Callout>
...
<table className="min-w-full border-collapse border border-gray-300 dark:border-gray-700 my-6">
  ...
  <tbody>
    <tr className="hover:bg-gray-50 dark:hover:bg-gray-800">
      ...
      <td className="border border-gray-300 dark:border-gray-700 px-4 py-2">❌ No (temporary)</td>
      <td className="border border-gray-300 dark:border-gray-700 px-4 py-2">✅ Yes (permanent)</td>
    </tr>
    <tr className="hover:bg-gray-50 dark:hover:bg-gray-800">
      ...
      <td className="border border-gray-300 dark:border-gray-700 px-4 py-2">✅ Yes (OS, kernel, etc.)</td>
    </tr>
    <tr className="hover:bg-gray-50 dark:hover:bg-gray-800">
      <td className="border border-gray-300 dark:border-gray-700 px-4 py-2">Manages hostname types</td>
      <td className="border border-gray-300 dark:border-gray-700 px-4 py-2">❌ No</td>
      <td className="border border-gray-300 dark:border-gray-700 px-4 py-2">✅ Yes (static/transient/pretty)</td>
    </tr>
    <tr className="hover:bg-gray-50 dark:hover:bg-gray-800">
      <td className="border border-gray-300 dark:border-gray-700 px-4 py-2">Works on all Linux?</td>
      <td className="border border-gray-300 dark:border-gray-700 px-4 py-2">✅ Yes (traditional)</td>
      <td className="border border-gray-300 dark:border-gray-700 px-4 py-2">✅ Yes (systemd systems)</td>
    </tr>
    <tr className="hover:bg-gray-50 dark:hover:bg-gray-800">
...
This command:
1. ✅ Updates the **static hostname** (writes to `/etc/hostname`)
2. ✅ Updates the **transient hostname** (active immediately)
3. ✅ **Survives reboot** (permanent change)
...
<Callout type="info">
💡 **Note**: Pretty hostnames can contain spaces, uppercase letters, and special characters—unlike static/transient hostnames which must follow strict naming rules.
</Callout>
...
<div className="my-8 p-6 bg-gradient-to-r from-purple-50 to-pink-50 dark:from-purple-900/20 dark:to-pink-900/20 rounded-lg border-2 border-purple-200 dark:border-purple-800">
  ...
  <div className="space-y-3 text-gray-700 dark:text-gray-300">
    ...
    <p className="text-sm italic mt-4">Traffic sent to localhost never leaves your computer—it loops back internally!</p>
  </div>
</div>
...
<Callout type="info">
💡 **Machine ID vs Hostname**: The hostname can change, but machine ID stays the same. Think of it as a "permanent name" while hostname is the "display name."
</Callout>
...
**Pro tip**: No reboot required—the change is immediate!
</details>
...
1. **Use Descriptive Names**
   ```
   ✅ prod-web-01, prod-db-primary
   ❌ server1, myserver
   ```

2. **Include Environment in Name**
   ```
   ✅ prod-app-01, staging-app-01, dev-app-01
   ```
...
<div className="my-8 p-6 bg-gradient-to-r from-green-50 to-emerald-50 dark:from-green-900/20 dark:to-emerald-900/20 rounded-lg border-2 border-green-200 dark:border-green-800">
  ...
  <ul className="space-y-2 text-gray-700 dark:text-gray-300">
    <li><strong>✓ hostname command</strong>: Traditional tool for viewing/setting hostnames (changes are temporary)</li>
    <li><strong>✓ hostnamectl command</strong>: Modern systemd tool for permanent hostname management (recommended)</li>
    <li><strong>✓ Three hostname types</strong>: Static (permanent in /etc/hostname), Transient (current runtime), Pretty (human-friendly)</li>
    <li><strong>✓ /etc/hostname file</strong>: Stores the static hostname (permanent across reboots)</li>
    <li><strong>✓ /etc/hosts file</strong>: Maps hostnames to IPs locally, overrides DNS</li>
    <li><strong>✓ localhost</strong>: Special hostname that always refers to this computer (127.0.0.1 or ::1)</li>
    <li><strong>✓ Naming rules</strong>: Use lowercase, numbers, hyphens only (1-63 chars)</li>
    <li><strong>✓ hostname -i</strong>: Shows all IP addresses for your hostname</li>
    <li><strong>✓ hostname -f</strong>: Shows FQDN (if configured)</li>
    <li><strong>✓ Machine ID</strong>: Unique, permanent identifier in /etc/machine-id</li>
  </ul>
</div>
...
Excellent work! You've mastered hostname management—a fundamental skill for Linux system administration. You can now:
- ✅ View hostnames using multiple methods
- ✅ Set temporary hostnames for testing
- ✅ Make permanent hostname changes with hostnamectl
- ✅ Understand static, transient, and pretty hostnames
- ✅ Configure local name resolution with /etc/hosts
- ✅ Follow professional hostname naming conventions

**In the next post (Part 17)**, we'll dive into the **Linux Filesystem Hierarchy**—understanding the structure and purpose of directories like `/etc`, `/var`, `/usr`, `/home`, and more. You'll learn why Linux organizes files the way it does and where to find critical system files.
...
<Callout type="success">
🎉 **Congratulations!** You've completed LFCS Phase 1 Part 16! You now understand how to manage Linux hostnames like a professional system administrator. These skills are essential for server management, networking, and the LFCS exam.
...
**Series Navigation:**
- ← Previous: [Part 15 - Understanding Network Interfaces with ip Command](/blog/2025-11-19_lfcs-phase1-part-15-network-interfaces-ip-command)
- → Next: Part 17 - Understanding Linux Filesystem Hierarchy (Coming Soon)
