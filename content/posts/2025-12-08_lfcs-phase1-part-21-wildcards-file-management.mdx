...
<div className="my-8 p-6 bg-gradient-to-r from-blue-50 to-purple-50 dark:from-blue-950 dark:to-purple-950 rounded-lg border-2 border-blue-200 dark:border-blue-800">
  ...
  <div className="flex flex-col space-y-4">
    ...
    <div className="text-center text-3xl text-blue-500">↓</div>
    <div className="bg-white dark:bg-gray-800 p-4 rounded-lg shadow border border-gray-200 dark:border-gray-700">
...
1. **Wildcards are shell features**, not command features—they work with ANY command
2. **`*` matches zero or more characters**—the most flexible wildcard
3. **`?` matches exactly one character**—useful for precise patterns
4. **`[...]` matches one character from a set**—powerful for specific ranges
5. **`[!...]` negates the match**—excludes specific characters
6. **Always test with `ls` before `rm`**—safety first!
7. **Hidden files (.dotfiles) need explicit `.*` pattern**—`*` won't match them
8. **Wildcards don't match across directory separators**—use `*/*` for nested dirs
9. **Quote or escape literal wildcard characters**—when files actually contain *, ?, [
10. **Combine wildcards for sophisticated patterns**—`file[0-9][0-9].txt`, `*backup*.tar.gz`
