...
<Callout type="error">
⚠️ **EXTREME CAUTION REQUIRED**: The `rm` command permanently deletes files with no recovery. One typo can destroy your entire system. This post will teach you safe practices and protective measures.
</Callout>
...
<Callout type="error">
🚨 **There is NO UNDO for rm**. Files deleted with `rm` are gone forever. Professional sysadmins have lost their jobs over a single wrong `rm` command. **Always think before you type.**
</Callout>
...
💡 **Pro Tip**: Many sysadmins create an alias: `alias rm='rm -i'` in their `.bashrc` to always prompt before deletion. This prevents accidental deletions.
</Callout>
...
⚠️ **rm -r is PERMANENT**: It deletes entire directory trees instantly. Always verify the path before hitting Enter!
</Callout>
...
<div className="grid grid-cols-1 md:grid-cols-2 gap-6 my-8">
  <div className="bg-white dark:bg-gray-800 p-6 rounded-lg border-2 border-green-200 dark:border-green-900">
    ...
    <ul className="space-y-2 text-sm">
      <li>✓ Only removes empty directories</li>
      <li>✓ Fails if directory has contents</li>
      <li>✓ Prevents accidental data loss</li>
      <li>✓ No recursive option</li>
      <li>✓ Safe by design</li>
      <li>✓ Cannot delete files</li>
      <li>✓ Good for cleanup scripts</li>
    </ul>
  </div>
  <div className="bg-white dark:bg-gray-800 p-6 rounded-lg border-2 border-red-200 dark:border-red-900">
    ...
    <ul className="space-y-2 text-sm">
      <li>✗ Deletes directories AND contents</li>
      <li>✗ Deletes everything recursively</li>
      <li>✗ High risk of data loss</li>
      <li>✗ Works on full and empty dirs</li>
      <li>✗ Dangerous by design</li>
      <li>✗ Can delete anything</li>
      <li>✗ Requires extreme caution</li>
    </ul>
  </div>
</div>
...
<Callout type="error">
🚨 **NEVER RUN THESE COMMANDS** - They will destroy your system!
</Callout>
...
<Callout type="warning">
⚠️ **Prevention is the only cure**: The ONLY reliable protection against `rm` is having backups. No backups = no recovery.
</Callout>
...
<Callout type="success">
🎯 **Safe practice environment**: Create a test directory for these labs so you can practice `rm` safely without risk to your system.
</Callout>
...
<Callout type="error">
⚠️ **Final Warning**: The `rm` command is the most dangerous tool you'll use in Linux. One mistake can cost you your data, your job, or even your entire system.
...
Practice these labs until safe deletion becomes muscle memory. Your future self (and your employer) will thank you! 🛡️
</Callout>
