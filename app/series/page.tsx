      ...
      <div className="max-w-[1600px] mx-auto px-4 sm:px-6 lg:px-8 py-12">
        {allSeries.length > 0 ? (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            {allSeries.map((series) => (
              <Link
                ...
                className="group relative overflow-hidden rounded-2xl border border-slate-200 dark:border-slate-700/50 bg-white/50 dark:bg-slate-800/50 hover:border-cyan-500/50 hover:bg-white dark:hover:bg-slate-800 transition-all duration-300"
              >
                {/* Gradient overlay */}
                ...
                <div className="relative p-8">
                  ...
                  <div className="flex items-start gap-5 mb-6">
                    ...
                    <div className="flex-1">
                      <h2 className="text-2xl font-bold text-slate-800 dark:text-slate-100 group-hover:text-cyan-600 dark:group-hover:text-cyan-400 transition-colors mb-2">
                        {series.meta.title}
                      </h2>
                  ...
                  {/* Stats Bar */}
                  <div className="flex items-center gap-6 mb-6 text-sm text-slate-500 dark:text-slate-400">
                    <div className="flex items-center gap-2">
                      <svg className="w-5 h-5 text-cyan-500" fill="currentColor" viewBox="0 0 20 20">
                        <path d="M9 4.804A7.968 7.968 0 005.5 4c-1.255 0-2.443.29-3.5.804v10A7.969 7.969 0 015.5 14c1.669 0 3.218.51 4.5 1.385A7.962 7.962 0 0114.5 14c1.255 0 2.443.29 3.5.804v-10A7.968 7.968 0 0014.5 4c-1.255 0-2.443.29-3.5.804V12a1 1 0 11-2 0V4.804z" />
                      </svg>
                      <span className="font-medium text-slate-600 dark:text-slate-300">{series.postCount} posts</span>
                    </div>
                    <div className="flex items-center gap-2">
                      <svg className="w-5 h-5 text-cyan-500" fill="currentColor" viewBox="0 0 20 20">
                  ...
                  <div className="flex items-center justify-between">
                    <span className="inline-flex items-center text-base font-medium text-cyan-600 dark:text-cyan-400 group-hover:text-cyan-500 dark:group-hover:text-cyan-300 transition-colors">
                      Start this track
          ...
          <div className="text-center py-20">
            <p className="text-slate-500 dark:text-slate-400 text-lg">No learning tracks available yet.</p>
          </div>
        )}

        {/* Coming Soon Section */}
        <div className="mt-16">
          <h2 className="text-2xl font-bold text-slate-800 dark:text-slate-100 mb-8 text-center">Coming Soon</h2>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {[
              ...
              <div
                key={upcoming.title}
                className="relative overflow-hidden rounded-xl border border-slate-200/30 dark:border-slate-700/30 bg-slate-100/30 dark:bg-slate-800/30 p-6 opacity-60"
              >
                ...
                <div className="absolute top-3 right-3">
                  <span className="text-xs bg-slate-200/50 dark:bg-slate-700/50 text-slate-500 dark:text-slate-400 px-2 py-1 rounded-full">
                    Coming Soon
                  </span>
                </div>
              </div>
