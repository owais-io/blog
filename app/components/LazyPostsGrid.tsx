...
export default function LazyPostsGrid({
  ...
  return (
    <section>
      {showTitle && (
        <h2 className="text-3xl sm:text-4xl font-bold text-slate-800 dark:text-slate-100 mb-10 tracking-tight">
        ...
        {visiblePosts.map((post) => (
          ...
          >
            <Link href={`/blog/${post.slug}`} className="block p-6">
              ...
              <h3 className="text-lg font-semibold text-slate-800 dark:text-slate-100 mb-3 group-hover:text-cyan-600 dark:group-hover:text-cyan-400 transition-colors line-clamp-2 leading-snug">
                {post.title}
              </h3>
              ...
              <div className="flex flex-wrap gap-1.5">
                {post.categories.slice(0, 2).map((category) => (
                  <span
                    key={category}
                    className="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium bg-green-100 dark:bg-green-900 text-green-700 dark:text-green-300"
                  >
                    {category}
                  </span>
                ))}
        ...
        <div className="mt-12">
          {loadType === 'button' ? (
            ...
            <div ref={loadMoreRef} className="h-20 flex flex-col items-center justify-center">
              ...
              <p className="text-xs text-slate-500 dark:text-slate-400">Loading more...</p>
            </div>
          )}
        </div>
      )}
      ...
      {loadedCount >= filteredPosts.length && filteredPosts.length > initialCount && (
        <div className="mt-12 text-center">
          <div className="inline-flex items-center gap-2 px-4 py-2 bg-green-100 dark:bg-green-900 text-green-700 dark:text-green-300 rounded-full text-sm font-medium">
      ...
      {filteredPosts.length === 0 && filterByCategory && (
        <div className="mt-12 text-center py-12">
          <div className="mx-auto w-24 h-24 mb-6 bg-slate-200 dark:bg-slate-700 rounded-full flex items-center justify-center">
