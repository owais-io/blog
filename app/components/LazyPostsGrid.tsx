...
export default function LazyPostsGrid({
  ...
  return (
    <section>
      {showTitle && (
        <h2 className="text-3xl sm:text-4xl font-bold text-slate-100 mb-10 tracking-tight">
        ...
        {visiblePosts.map((post, index) => (
          ...
          >
            <Link href={`/blog/${post.slug}`} className="block p-6">
              ...
              <h3 className="text-lg font-semibold text-slate-100 mb-3 group-hover:text-cyan-400 transition-colors line-clamp-2 leading-snug">
                {post.title}
              </h3>
      ...
      {loadedCount >= filteredPosts.length && filteredPosts.length > initialCount && (
        <div className="mt-12 text-center">
          <div className="inline-flex items-center gap-2 px-4 py-2 bg-green-900 text-green-300 rounded-full text-sm font-medium">
      ...
      {filteredPosts.length === 0 && filterByCategory && (
        <div className="mt-12 text-center py-12">
          <div className="mx-auto w-24 h-24 mb-6 bg-slate-700 rounded-full flex items-center justify-center">
