        ...
        <div className="lg:col-span-3">
          ...
          <div className="lg:hidden mb-6">
            <button
              ...
              className="w-full flex items-center justify-between px-4 py-3 bg-white dark:bg-slate-700 border border-slate-200 dark:border-slate-600 rounded-lg hover:bg-slate-100 dark:hover:bg-slate-500 transition-colors"
            >
          ...
          {(selectedTags.length > 0 || selectedCategories.length > 0) && (
            <div className="bg-cyan-100 dark:bg-cyan-900/30 border border-cyan-300 dark:border-cyan-700 rounded-lg p-4 mb-6">
              <div className="flex items-center justify-between mb-3">
                ...
                <button
                  ...
                  className="text-xs text-cyan-600 dark:text-cyan-400 hover:text-cyan-500 dark:hover:text-cyan-300 font-medium flex items-center gap-1"
                >
              ...
              <div className="flex flex-wrap gap-2">
                {selectedCategories.map((category) => (
                  <span
                    key={category}
                    className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full text-sm font-medium bg-green-100 dark:bg-green-900/50 text-green-700 dark:text-green-300 border border-green-300 dark:border-green-700"
                  >
                    <span className="text-xs">📁</span>
                    {category}
                    <button
                      onClick={() => handleCategoryToggle(category)}
                      className="ml-1 hover:text-green-600 dark:hover:text-green-200 font-bold"
                ...
                {selectedTags.map((tag) => (
                  <span
                    key={tag}
                    className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full text-sm font-medium bg-cyan-100 dark:bg-cyan-900/50 text-cyan-700 dark:text-cyan-300 border border-cyan-300 dark:border-cyan-700"
                  >
                    <span className="text-xs">#</span>
                    {tag}
                    <button
                      onClick={() => handleTagToggle(tag)}
                      className="ml-1 hover:text-cyan-600 dark:hover:text-cyan-200 font-bold"
          ...
          {filteredPosts.length === 0 ? (
            <div className="text-center py-12">
              ...
              <button
                ...
                className="mt-4 inline-flex items-center text-cyan-600 dark:text-cyan-400 hover:text-cyan-500 dark:hover:text-cyan-300"
              >
                ← Clear all filters
              </button>
            </div>
